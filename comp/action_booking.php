...
$strSQL = "INSERT INTO bookingRoom_2(
    `id`,
    `start`,
    `end`,
    `title`,
...
if ($objQuery) {
    $strline = "SELECT * FROM bookingRoom_2 INNER JOIN user ON bookingRoom_2.U_id=user.U_id ORDER BY id DESC LIMIT 1";
    ...
    echo '<script>alert("บันทึกการจอง และแจ้งเตือน Admin แล้ว");window.location="./";</script>';
} else {
